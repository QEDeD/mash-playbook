...
The simplest fix is to connect to the server once (e.g. `ssh root@mash.example.com`) and confirm the host key. Ansible runs after that will find it in your `known_hosts` file.
