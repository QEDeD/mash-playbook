...
```text
one managed node (one Linux server or VM)
├─ Unix socket path
│  └─ host directory containing the socket bind-mounted into both containers
└─ TCP path
...
```text
Nextcloud container
└─ local Docker network on managed node A
   └─ inter-node network and security boundary
      └─ reachable TCP endpoint on managed node B
         └─ Valkey container
```

A remote Valkey deployment therefore requires TCP, but choosing TCP does not make Valkey remote: both TCP recipes below keep Nextcloud and Valkey on the same managed node. To learn more about Nextcloud memory caching, see the [Nextcloud documentation](https://docs.nextcloud.com/server/latest/admin_manual/configuration_server/caching_configuration.html#id2).
...
Do not rely on an unscoped installation command to serialize these runs: Ansible may run inventory aliases which target the same managed node in parallel. See [Running multiple instances of the same service](../running-multiple-instances.md#installation) for the execution model. If you chose the shared or no-Valkey layout, run the normal installation for the main inventory host.
...
After changing the integration, rerun the [installation](#installation) and then run the configuration-adjustment command shown above. The installation updates the container environment, mounts, networks, and systemd dependencies; the adjustment updates or removes Nextcloud's persisted Redis and memory-cache settings.
