...
When running `just install-all`, `just install-service`, or a raw `ansible-playbook` command that combines an `install-*` tag with `start` or `start-group`, only services whose configuration or container image actually changed during the playbook run will be restarted. Unchanged services are left running (or get started if they were stopped). This reduces unnecessary downtime.

When running with `setup-*` tags together with a start or restart tag (e.g. `setup-all,start` or `setup-miniflux,start-group`), all targeted services are unconditionally restarted regardless of whether changes were detected. This is appropriate for setup's thorough "full setup" semantics.

Standalone `start`, `start-all`, `start-group`, `restart`, `restart-all`, and `restart-group` runs always restart all targeted services, since no installation tasks run during these commands.
